...
import { collection, query, where, getDocs, limit, DocumentData, QuerySnapshot } from "firebase/firestore";
...
export default async function ProductDetails({params}: {params: Promise<{ slug: string }>;}) {
  ...
  try {
    if (!slug) {
      error = "Product slug is missing from the URL or could not be determined.";
    } else {
      const productsCollectionRef = collection(db, "products"); 
      const q = query(productsCollectionRef, where("slug", "==", slug), limit(1));
    ...
    error = "Failed to load product details due to a server error. Please try again later.";
  }
  ...
  if (error) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[calc(100vh-200px)] text-red-600 bg-red-50 p-8 rounded-lg shadow">
        ...
        <p>{error}</p>
        <BackButton text="Back to Shop" className="mt-6 px-6 py-2 bg-blue-500 text-white rounded-md hover:bg-blue-600 transition-colors" />
      </div>
    );
  }
  ...
  return (
    <div className="bg-white p-6 sm:p- rounded-xl shadow-xl container mx-auto ">
      <BackButton text="Back to Previous Page" className="mb-6" />
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8 sm:gap-12">
        ...
        <div className="md:col-start-2 md:row-start-1">
          ...
          {product.regularPrice > 0 && product.salePrice > 0 && product.regularPrice > product.salePrice && (
            <p className="text-gray-500 text-base line-through">${product.regularPrice.toFixed(2)}</p>
          )}
          <p className="text-2xl sm:text-3xl font-semibold text-blue-600 mb-6">
            ${(product.salePrice > 0 ? product.salePrice : product.regularPrice).toFixed(2)}
          </p>
          
          <p className="text-gray-700 leading-relaxed mb-6">{product.description || "No description available for this product."}</p>
